...
class FlashSelfAttention(nn.Module):
    ...
    def flash_inner(self, qkv, pad_mask):
        return self.inner_attn(
            qkv, key_padding_mask=pad_mask, need_weights=False, causal=False
        )
    ...
    def forward(
        ...
        qkv = rearrange(
            qkv, "b s three (h d) -> b s three h d", h=self.config.num_attention_heads
        ).type(torch.float16)
        context, attn_weights = self.flash_inner(qkv, pad_mask=attention_mask)
        context_layer = rearrange(context, "b s h d -> b s (h d)").type(torch.float32)
