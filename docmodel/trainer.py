...
from etl_utils import unnormalize_bbox
...
class CustomTrainer(Trainer):
    ...
    def log_all_losses(self, outputs):
        # Warning -- pretty noisy
        prefix = getattr(self, "log_prefix", None) or "train"
        self.log(
            {
                f"{prefix}_loss": outputs["loss"].item(),
                f"{prefix}_mlm_loss": outputs["mlm_loss"].item(),
                f"{prefix}_position_loss": outputs["position_loss"]
                if isinstance(outputs["position_loss"], float)
                else outputs["position_loss"].item(),
            }
        )
